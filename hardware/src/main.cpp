...
// no connection to wifi posible
    ...
    if (success)
    {
        ...
        if (MDNS.begin("solax"))
        {
            WEB_DEBUG_PRINT("mDNS responder started")
        }
        else
        {
            WEB_DEBUG_PRINT("Error setting up MDNS responder!")
        }
    ...
    server.begin();
    WEB_DEBUG_PRINT("HTTP server started")
}
...
// handle mqtt
    ...
    {
        bool response = Inverter.requestInverterData();
